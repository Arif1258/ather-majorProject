...
async function sendEmailAlert(subject, message) {
  try {
    ...
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
      console.warn("[Cron] EMAIL_USER or EMAIL_PASS not set in .env. Cannot send email.");
      return;
    }
    ...
    console.log(`[Cron] Alert email sent to ${config.alertEmail}`);
  } catch (error) {
    console.error('[Cron] Failed to send email alert:', error.message);
  }
}
...
async function axiosWithRetry(url, options = {}, retries = 3, delayMs = 2000) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.log(`[Cron] Request failed for ${url}. Retrying in ${delayMs}ms (Attempt ${i + 1}/${retries})...`);
...
async function runChecks() {
  ...
  try {
    ...
    for (const site of websites) {
      ...
      try {
        const response = await axiosWithRetry(site.url, { timeout: 10000 });
        status = 'UP';
      ...
      if (prevStatus === 'UP' && status === 'DOWN') {
        const msg = `ALERT: ${site.url} went DOWN! Reason: ${errorMsg}`;
        logSystem(msg, 'Critical');
        await sendEmailAlert(`Website Down Alert: ${site.url}`, msg);
