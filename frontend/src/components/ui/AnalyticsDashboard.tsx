    ...
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-1000 max-w-[1600px] mx-auto pt-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-white/10 pb-6">
        ...
        <div className="flex flex-wrap items-center gap-4 bg-black/40 p-2 rounded-2xl border border-white/10">
          ...
          <button 
            onClick={() => window.location.href = `/api/websites/export${websiteId !== 'all' ? `?websiteId=${websiteId}` : ''}`} 
            className="ml-2 flex items-center gap-2 px-3 py-1.5 bg-brand-cyan/10 hover:bg-brand-cyan/20 text-brand-cyan border border-brand-cyan/20 rounded-md text-xs font-semibold transition-colors"
          >
